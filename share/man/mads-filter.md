...
3. the plugin file's stem (e.g. `bridge.plugin` → `bridge`).
...
* each agent is launched with a custom name: e.g. `mads filter -n my_plugin_x86` on X86 linux, `mads filter -n my_plugin_arm64` on ARM64 linux;
