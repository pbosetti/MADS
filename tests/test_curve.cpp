...
//
// Keys are generated at runtime with Mads::generate_keypair() and
...
TEST_CASE("fetch_public_keys succeeds when .pub files are present",
          "[curve]") {
  TempKeyDir dir;
  auto kp1 = Mads::generate_keypair();
  auto kp2 = Mads::generate_keypair();
...
TEST_CASE("full CURVE-encrypted REQ/REP round trip with ZAP authentication",
  ...
  zmq::socket_t server_sock(server_ctx, zmq::socket_type::rep);
  server_auth.setup_curve_server(server_sock, "server");
  server_sock.set(zmq::sockopt::rcvtimeo, 200);
  ...
  zmq::socket_t client_sock(client_ctx, zmq::socket_type::req);
  client_auth.setup_curve_client(client_sock, "client", "server");
  client_sock.set(zmq::sockopt::rcvtimeo, 3000);
  client_sock.connect(mads_test::loopback(port));

  zmq::multipart_t request;
  request.addstr("ping");
  REQUIRE(request.send(client_sock));

  bool server_got = mads_test::wait_for(
      [&] {
        zmq::multipart_t in;
        if (!in.recv(server_sock)) return false;
        REQUIRE(in.at(0).to_string() == "ping");
        zmq::multipart_t reply;
        reply.addstr("pong");
        reply.send(server_sock);
        return true;
      },
      3000ms, 50ms);
  REQUIRE(server_got);
  ...
  bool client_got = response.recv(client_sock);
  REQUIRE(client_got);
  REQUIRE(response.at(0).to_string() == "pong");
}
